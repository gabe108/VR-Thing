...
namespace VrGrabber
{
    ...
    public class VrgGrabber : MonoBehaviour
    {
        ...
        public Transform m_grabbedObject;
        ...
        void Update()
        {
            ...
            m_joystick = OVRInput.Get(OVRInput.Axis2D.PrimaryTouchpad);

            if (m_grabbedObject != null)
            {
                Vector3 f = objectTO.transform.forward * m_speed * m_joystick.x * Time.deltaTime;
                Vector3 r = objectTO.transform.right * m_speed * m_joystick.y * Time.deltaTime;
                f.y = 0;
                r.y = 0;
                float temp = m_grabbedObject.position.y;
                m_grabbedObject.Translate(f);
                m_grabbedObject.Translate(r);
                m_grabbedObject.position.Set(m_grabbedObject.position.x, temp, m_grabbedObject.position.z);
            }
        }
